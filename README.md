...
*💡 命名参数：以上所有方法在 `SimpleJdbcTemplate` 上均有三种参数模式的重载：① 直接传参 — 直接传入 SQL 字符串 + `Map`（每次解析）；② 模板传参 — 先解析为 `NamedParamSql`，后续每次调用传入模板 + `Map`（解析一次、绑参多次）；③ 预构建传参 — 传入 `PreparedSql`（参数已绑死）。详见「[7.3 命名参数构建](#73-命名参数构建)」。*
